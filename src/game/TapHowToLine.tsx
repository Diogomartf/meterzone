...
/**
 * First-play how-to. Visible on the first frame after PLAY, then fades out
 * 4s later. Stays mounted across levels so that timer is not reset.
 */
export function TapHowToLine({ visible }: { visible: boolean }) {
  const m = useMessages();
  // Starts fully visible so the tip is there on the first frame after PLAY.
  ...
  useEffect(() => {
    ...
    // On-screen immediately after PLAY, then hold and fade.
    opacity.value = 1;
